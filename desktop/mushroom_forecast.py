...
# --------------------------------------------------------------------------- #
#
# Отдельная модель, а не ещё несколько строк в SPECIES. Гриб в этой модели —
# отклик на импульс: дождь проходит через окно задержки (lag_kernel), и через
# несколько дней после КОНКРЕТНОГО увлажнения появляется волна плодовых тел.
# Ягода растёт не так. Черника не появляется через неделю после дождя — она
# наливается весь сезон от одной даты цветения, и по погоде by это не серия
# волн, а один процесс: зацвело, наливается, поспело, держится, осыпалось.
# Заморозок в цвету не откладывает урожай на потом, а убирает его целиком —
# новых цветков в этом году больше не будет. Засуха в наливе не убирает ягоду,
# а делает её мельче и суше. Это два разных явления, и общий с грибами код
# (rain_pulse, lag_kernel) для них не подходит: попытка натянуть его дала бы
# график с несуществующими волнами после каждого дождя.
#
# Фенология считается тем же способом, что и у весенних грибов (сморчок,
# строчок) — через накопленное тепло от схода снега (snowmelt_gdd), только
# ищет не сам «слой», а дату цветения; дальше срок до спелой ягоды и её
# сохранности отсчитывается уже календарными сутками от этой даты, а не новым
# накоплением тепла — так короче и понятнее объяснить человеку.
...
class Berry:
    ...
    name: str
    latin: str
    flower_gdd_opt: float     # накопленное тепло от схода снега до цветения, °C·сут
    flower_gdd_sigma: float   # ширина окна цветения по накопленному теплу
    frost_window_days: int    # сколько суток вокруг цветения опасен заморозок
    frost_kill_c: float       # ночной минимум, губящий цветки, °C
    ripen_days: int           # от цветения до первых спелых ягод, сут
    ripen_span_days: int      # сколько держится хороший сбор, пока не осыпалось
    m_min: float              # влагозапас, ниже которого налив останавливается
    m_opt: float              # влагозапас насыщения для полноценной ягоды
    frost_hardy_fruit: bool = False   # спелая ягода не боится заморозка (клюква)
    note: str = ""


BERRIES: dict[str, Berry] = {
    "черника": Berry(
        "Черника", "Vaccinium myrtillus", 230.0, 60.0, 6, -2.5,
        50, 28, 0.25, 0.55,
        note="Цветёт вскоре после схода снега, спелая с середины июля. "
             "Заморозок в цвету на несколько градусов ниже нуля губит "
             "урожай сразу на весь сезон — новых цветков не будет."),
    "брусника": Berry(
        "Брусника", "Vaccinium vitis-idaea", 250.0, 65.0, 6, -3.0,
        80, 35, 0.15, 0.42,
        note="Цветёт позже черники и наливается дольше — сбор начинается "
             "в августе. Переносит более сухую почву, держится на кусте "
             "хорошо: собирать можно не спеша."),
    "малина": Berry(
        "Малина", "Rubus idaeus", 420.0, 70.0, 5, -2.0,
        32, 38, 0.22, 0.58,
        note="Цветёт в июне — заморозок в эту пору редок, но возможен. "
             "От дождя ягода наливается быстрее, а в сушь мельчает. "
             "Любит вырубки, гари и опушки, в глухом лесу почти не растёт."),
    "клюква": Berry(
        "Клюква", "Oxycoccus palustris", 300.0, 70.0, 6, -2.5,
        105, 40, 0.45, 0.75, frost_hardy_fruit=True,
        note="Растёт только на верховом болоте, вне его — редкость. Спеет "
             "поздно, в сентябре-октябре, и в отличие от прочих ягод "
             "спелому плоду заморозок не вредит: после первых холодов "
             "клюква становится только слаще."),
}


for _k, _b in BERRIES.items():
    _b.key = _k


def berry_index(b: Berry, days: list[Day], m: list[float], ts: list[float]) -> list[float]:
    """Индекс ягоды 0..100: фенология вместо отклика на импульс.

    Ноль до цветения — не пропуск данных, а честный ответ «ягоды ещё нет»,
    поэтому в отличие от species_index здесь не NaN, а 0.0: недостатка
    истории для расчёта тут не бывает, есть просто «ещё рано».
    """
    gdd, melt = snowmelt_gdd(days, ts)
    if melt is None:
        # Не удалось определить сход снега вообще — фенологию строить не от
        # чего, и гадать константой, как для грибов (0.35), здесь не годится:
        # у ягоды это не один из множителей, а сама основа расчёта.
        return [0.0] * len(days)

    flower_i = next((i for i, g in enumerate(gdd) if g >= b.flower_gdd_opt), None)

    killed = False
    if flower_i is not None:
        lo = max(0, flower_i - b.frost_window_days)
        hi = min(len(days) - 1, flower_i + b.frost_window_days)
        killed = any(days[j].tmin <= b.frost_kill_c for j in range(lo, hi + 1))

    ripe_full = b.ripen_days + 6           # неделя на дозревание до полной кондиции
    fade_start = ripe_full + b.ripen_span_days
    fade_end = fade_start + 10             # осыпается или подгнивает за полторы недели

    out = []
    for i in range(len(days)):
        if flower_i is None or i < flower_i or killed:
            out.append(0.0)
            continue

        age = i - flower_i
        curve = _ramp(age, b.ripen_days, ripe_full) * (1.0 - _ramp(age, fade_start, fade_end))
        if curve <= 0:
            out.append(0.0)
            continue

        lo_w, hi_w = flower_i, min(i, flower_i + b.ripen_days)
        m_avg = sum(m[j] for j in range(lo_w, hi_w + 1)) / (hi_w - lo_w + 1)
        f_m = _ramp(m_avg, b.m_min, b.m_opt)

        f_frost = 1.0
        if not b.frost_hardy_fruit:
            tmin = days[i].tmin
            if tmin < -4:
                f_frost = 0.15
            elif tmin < -1:
        ...
        f_bio = biotope_weight(b)
        out.append(100.0 * min(1.0, curve * f_m * f_frost * f_bio))
    return out


def berry_explain(b: Berry, i: int, days: list[Day], m: list[float],
                  ts: list[float]) -> list[tuple[str, float, str]]:
    """Разложение индекса ягоды на факторы — тот же вид ответа, что explain()
    даёт для гриба, чтобы в интерфейсе не пришлось различать два формата."""
    gdd, melt = snowmelt_gdd(days, ts)
    if melt is None:
        return [("Дата цветения", 0.0, "не удалось определить сход снега")]

    flower_i = next((j for j, g in enumerate(gdd) if g >= b.flower_gdd_opt), None)
    out = []
    if flower_i is None:
        out.append(("Цветение", 0.0,
                    f"тепла от схода снега накоплено {gdd[i]:.0f} °C·сут из "
                    f"нужных {b.flower_gdd_opt:.0f} — ещё не цвело"))
        return out

    out.append(("Цветение", 1.0 if i >= flower_i else 0.0,
               f"снег сошёл {melt.strftime('%d.%m')}, зацвело примерно "
               f"{(melt and days[flower_i].d.strftime('%d.%m')) or '—'}"))

    lo = max(0, flower_i - b.frost_window_days)
    hi = min(len(days) - 1, flower_i + b.frost_window_days)
    killed = any(days[j].tmin <= b.frost_kill_c for j in range(lo, hi + 1))
    out.append(("Заморозок в цвету", 0.0 if killed else 1.0,
               f"опасен минимум ниже {b.frost_kill_c:.1f} °C в окне "
               f"±{b.frost_window_days} сут вокруг цветения"
               + (" — урожай этого сезона погиб" if killed else "")))

    if i >= flower_i and not killed:
        age = i - flower_i
        ripe_full = b.ripen_days + 6
        fade_start = ripe_full + b.ripen_span_days
        fade_end = fade_start + 10
        curve = _ramp(age, b.ripen_days, ripe_full) * (1.0 - _ramp(age, fade_start, fade_end))
        out.append(("Срок налива", curve,
                   f"{age} сут после цветения, спелость — с {b.ripen_days} по "
                   f"{fade_start} сутки"))
        lo_w, hi_w = flower_i, min(i, flower_i + b.ripen_days)
        m_avg = sum(m[j] for j in range(lo_w, hi_w + 1)) / (hi_w - lo_w + 1)
        out.append(("Влага в наливе", _ramp(m_avg, b.m_min, b.m_opt),
                   f"в среднем за налив {m_avg * 100:.0f}% ёмкости, нужно "
                   f"от {b.m_min * 100:.0f}%"))
        if not b.frost_hardy_fruit:
            tmin = days[i].tmin
            f_frost = 0.15 if tmin < -4 else (0.55 if tmin < -1 else 1.0)
            out.append(("Заморозок на спелой ягоде", f_frost,
                       "мельчит и портит ягоду, в отличие от клюквы"))
    bw = biotope_weight(b)
    out.append((f"Тип леса: {CURRENT_BIOTOPE.name.lower()}", min(1.0, bw),
               "у ягоды свои предпочтения по типу леса, как и у гриба"))
    return out
...
BIOTOPES: dict[str, Biotope] = {
    "смешанный": Biotope(
        ...
        {"черника": 1.00, "брусника": 1.00, "малина": 0.60, "клюква": 0.10},
    ...
    "сосняк": Biotope(
        ...
        {"маслёнок": 1.30, "лисичка": 1.15, "белый": 1.00, "сыроежка": 1.00,
         ...
         "черника": 0.90, "брусника": 1.20, "малина": 0.50, "клюква": 0.10},
    ...
    "березняк": Biotope(
        ...
        {"подберёзовик": 1.30, "белый": 1.00, "подосиновик": 1.00, "груздь": 1.15,
         ...
         "черника": 0.60, "брусника": 0.50, "малина": 0.90, "клюква": 0.10},
    ...
    "ельник": Biotope(
        ...
        {"белый": 1.05, "груздь": 1.10, "лисичка": 0.85, "опёнок": 1.10,
         ...
         "черника": 1.10, "брусника": 0.80, "малина": 0.25, "клюква": 0.10},
    ...
    "низина": Biotope(
        ...
        {"груздь": 1.25, "опёнок": 1.20, "вешенка": 1.20, "подосиновик": 1.05,
         ...
         "черника": 0.70, "брусника": 0.40, "малина": 0.60, "клюква": 0.30},
        note="Влага держится долго, зато в сушь тут единственное место с грибами; "
             "в мокрый год, наоборот, заливает."),
    "болото": Biotope(
        "болото", "Верховое болото", 0.28, 0.55, 120.0, 0.95, -1.0,
        {"клюква": 1.50, "брусника": 0.50, "черника": 0.30, "малина": 0.10,
         "белый": 0.10, "подберёзовик": 0.20, "подосиновик": 0.10,
         "лисичка": 0.10, "маслёнок": 0.10, "опёнок": 0.10, "груздь": 0.10,
         "сыроежка": 0.15, "вешенка": 0.10, "сморчок": 0.10},
        note="Открытый мох и стоячая вода: почва почти всегда насыщена влагой "
             "и холоднее, чем в лесу. Грибам тут почти нет места, зато это "
             "единственный дом клюквы."),
    "вырубка": Biotope(
        "вырубка", "Вырубка, гарь, опушка", 0.06, 0.24, 30.0, 0.90, 1.5,
        {"малина": 1.40, "черника": 0.20, "брусника": 0.30, "клюква": 0.10,
         "лисичка": 1.10, "белый": 0.60, "подберёзовик": 0.70,
         "подосиновик": 0.60, "маслёнок": 0.40, "опёнок": 0.50,
         "груздь": 0.25, "сыроежка": 0.70, "вешенка": 0.20, "сморчок": 0.40},
        note="Открытое место без полога: солнце прогревает почву быстрее, "
             "но она же быстрее сохнет. Малина здесь в своей стихии, "
             "лесные грибы — не очень."),
}
...
def berry_plain_summary(b: Berry, i: int, days: list[Day], m: list[float],
                        ts: list[float], value: float) -> str:
    """Объяснение прогноза ягоды обычными словами — то же самое, что
    plain_summary делает для гриба, но по факторам фенологии, а не по
    «сколько дней с дождя»: у ягоды это просто не тот вопрос."""
    fx = berry_explain(b, i, days, m, ts)
    weak = sorted(fx, key=lambda x: x[1])

    if value >= 68:
        lead = f"{b.name}: сбор в самом разгаре."
    elif value >= 33:
        lead = f"{b.name}: ягода есть, но не пик сезона."
    elif value >= 18:
        lead = f"{b.name}: только начинается или уже на исходе."
    else:
        lead = f"{b.name}: рано, поздно, или сезон в этом году не задался."

    good = [why for _, v, why in fx if v >= 0.85][:2]
    bad = [f"{n.lower()}: {why}" for n, v, why in weak[:2] if v < 0.6]
