...
class DayRow(Card):
    ...
    def __init__(self, res, i, name, on_tap, **kw):
        ...
        box.add_widget(Label(text=f"{v:.0f}", color=fg, font_size=sp(20), bold=True))
        self.add_widget(box)
...
class MushroomApp(App):
    ...
    def on_pause(self):
        """Разрешает Android приостановить приложение вместо убийства.

        Без этого метода поведение по умолчанию в Kivy и так True — но
        явный override рядом с on_resume() понятнее, чем расчёт на
        умолчание, которое человек, читающий код через полгода, не обязан
        помнить.
        """
        return True

    def on_resume(self):
        """Снимает «застрявшие» касания при возврате из фона.

        Если экран гаснет или приложение сворачивается прямо в момент
    ...
    def _build_ui(self):
        ...
        # Secondary controls share one rounded treatment in both themes.
        for control in (self.btn_place, b_gps, b_star, self.b_theme, b_help):
            uikit.style_secondary(control, CARD, control.color)
        if self.b_premium is not None:
            uikit.style_secondary(self.b_premium, CARD, MUTED)
        parts["top"] = top
        parts["tools"] = tools
        ...
        uikit.style_secondary(self.sp_days, CARD, INK)
        ...
        # Основные действия визуально отделены от полей выбора.
        for action, color in ((self.btn, ACCENT), (b_walk, BLUE)):
            action.background_color = (0, 0, 0, 0)
            action.color = hexc(palette.ON_DARK)
            uikit.fill_rounded(action, color)
            uikit.press_feedback(action, color)
        parts["row2"] = row2
        ...
        self.card = Card(orientation="vertical", size_hint_y=None, height=dp(88),
                         padding=(dp(14), dp(10)), spacing=dp(4))
        self.l_main = Label(text="Данные не загружены", font_size=sp(23), bold=True,
        ...
        saved = prefs.load()
        # На узком телефоне три Spinner и «Радар» в одном ряду получали
        # по 60–75 dp: подписи выходили за границы кнопок. Две строки дают
        # виду всю доступную ширину, а лесу и рельефу — по половине.
        picks = BoxLayout(orientation="vertical", size_hint_y=None,
                          height=2 * TOUCH + dp(6), spacing=dp(6))
        pick_species = BoxLayout(size_hint_y=None, height=TOUCH, spacing=dp(6))
        pick_place = BoxLayout(size_hint_y=None, height=TOUCH, spacing=dp(6))
        ...
        self.sp_kind = Spinner(text=self._saved_kind(saved), font_size=sp(13),
                               background_normal="", background_color=CARD, color=INK,
        ...
        self.sp_relief = Spinner(text=self._saved_relief(saved), font_size=sp(12),
                                 background_normal="", background_color=CARD, color=INK,
                                 values=[r.name for r in engine.RELIEFS.values()])
        self.sp_relief.bind(text=self._on_relief)
        # Подпись сокращается только при отрисовке: полное выбранное значение
        # остаётся в Spinner.text и продолжает передаваться в расчёт.
        # Внутренний отступ не даёт буквам прилипать к границе кнопки.
        for selector in (self.sp_kind, self.sp_bio, self.sp_relief):
            selector.shorten = True
            selector.shorten_from = "right"
            selector.halign = "center"
            selector.valign = "middle"
            selector.bind(size=lambda widget, size: setattr(
                widget, "text_size", (max(dp(24), size[0] - dp(24)), size[1])))
            # Прямоугольная область касания остаётся прежней, скругляется
            # только фон. Цвет берётся из текущей палитры при сборке экрана.
            uikit.style_secondary(selector, CARD, INK)
        if self.sp_kind.text != self.ALL_KINDS:
            self.sel = self.sp_kind.text
        pick_species.add_widget(self.sp_kind)
        b_radar = Button(text="Радар", size_hint_x=None, width=dp(82), font_size=sp(13),
                         bold=True, background_normal="", background_color=BLUE)
        b_radar.bind(on_release=lambda *_: self.show_radar())
        b_radar.color = hexc(palette.ON_DARK)
        b_radar.background_color = (0, 0, 0, 0)
        uikit.fill_rounded(b_radar, BLUE)
        uikit.press_feedback(b_radar, BLUE)
        pick_species.add_widget(b_radar)
        pick_place.add_widget(self.sp_bio)
        pick_place.add_widget(self.sp_relief)
        picks.add_widget(pick_species)
        picks.add_widget(pick_place)
        parts["picks"] = picks

        # Быстрый переход к календарю: на компактном экране дни ниже графика.
        # Карта и её управление остаются без изменений.
        calendar_jump = Button(text="К календарю", size_hint_y=None,
                               height=TOUCH, font_size=sp(13), bold=True)
        uikit.style_secondary(calendar_jump, CARD, INK)
        calendar_jump.bind(on_release=lambda *_: self.jump_to_calendar())
        parts["calendar_jump"] = calendar_jump
    ...
    def _arrange(self, parts):
        ...
        if not self._wide:
            # The growing header (two selector rows, larger forecast card and
            # chart) must not consume the entire viewport.  Scroll the whole
            # portrait page instead of giving the day list only the leftover
            # height, which could be zero on smaller Android screens.
            ...
            for key in ("top", "tools", "spots", "row2", "card", "picks",
                        "calendar_jump", "chart", "scale"):
                content.add_widget(parts[key])
            # Avoid nested vertical ScrollViews: the calendar's day rows are
            # part of the same page, so a single swipe reaches every day.
            if self.list.parent is not None:
                self.list.parent.remove_widget(self.list)
            content.add_widget(self.list)
            content.add_widget(parts["status"])
        ...
        for key in ("spots", "row2", "picks", "calendar_jump", "list"):
    ...
    def jump_to_calendar(self):
        """Прокрутить главный экран к дням без изменения карты или прогноза."""
        if not getattr(self, "_wide", False) and isinstance(self.root, ScrollView):
            # Kivy вычисляет размеры после следующего кадра; scroll_to
            # надёжнее прямого scroll_y при динамической высоте списка.
            Clock.schedule_once(lambda _dt: self.root.scroll_to(
                self.list, padding=dp(8), animate=True), 0)
        else:
            # В альбомной ориентации список уже виден в правой колонке.
            days_scroll = self.list.parent
            if isinstance(days_scroll, ScrollView):
                days_scroll.scroll_y = 1
    ...
    def calculate(self):
        ...
        self._spin_stop = uikit.spin_status(self.status, "Запрос погодных данных")
    ...
    def refresh(self):
        ...
        self.l_main.text = f"{v:.0f}/100  ·  {engine.level(v)}"
        ...
        self.l_sub.text = (f"{who}  ·  {sub}" if who and sub and who not in sub else (sub or engine.level(v)))
        self.l_sub.color = fg
        self.card.set_bg(bg)
    ...
    def show_radar(self):
        ...
        if not entries:
            ...
            return
        # Компактная матрица: вид слева, семь дней в фиксированных колонках.
        # Миниатюры видовые; цветные точки показывают индекс независимо от вида.
        content = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(8),
                            size_hint_y=None)
        content.bind(minimum_height=content.setter("height"))
        dates = BoxLayout(size_hint_y=None, height=dp(30), spacing=dp(2))
        dates.add_widget(Label(text="Вид / пик", size_hint_x=None, width=dp(100),
                               color=MUTED, font_size=sp(10)))
        for offset in range(7):
            date = r.days[r.today + offset].d.strftime("%a\n%d.%m") if r.today + offset < len(r.days) else ""
            dates.add_widget(Label(text=date, color=MUTED, font_size=sp(9)))
        content.add_widget(dates)
        for entry in entries:
            line = BoxLayout(size_hint_y=None, height=dp(54), spacing=dp(2))
            heading = BoxLayout(orientation="vertical", size_hint_x=None,
                                width=dp(100), spacing=dp(1))
            species_icon = species_art.icon_path(entry["name"])
            title_row = BoxLayout(spacing=dp(3))
            if species_icon:
                title_row.add_widget(Image(source=species_icon, size_hint_x=None,
                                           width=dp(30)))
            title = Label(text=entry["name"], color=INK, font_size=sp(10),
                          bold=True, shorten=True, shorten_from="right")
            title_row.add_widget(title)
            heading.add_widget(title_row)
            heading.add_widget(Label(text=f"{entry['phase']} · {entry['best']:.0f}",
                                     color=MUTED, font_size=sp(9)))
            line.add_widget(heading)
            for offset in range(7):
                cell = BoxLayout(orientation="vertical", padding=(dp(1), dp(7)))
                if offset < len(entry["values"]):
                    grade = radar.icon_grade(entry["values"][offset])
                    cell.add_widget(Image(source=os.path.join(os.path.dirname(__file__),
                                                              f"radar_{grade}.png"),
                                          allow_stretch=True, keep_ratio=True))
                line.add_widget(cell)
            content.add_widget(line)
        hint = Label(text="Цвет показывает индекс модели, а не вероятность находки.",
                     color=MUTED, font_size=sp(10), size_hint_y=None,
                     height=dp(44), halign="left")
        hint.bind(width=lambda w, v: setattr(w, "text_size", (v, None)))
        content.add_widget(hint)
        legend = Label(text="Серый — низкий  ·  жёлтый — средний  ·  зелёный — высокий",
                       color=MUTED, font_size=sp(10), size_hint_y=None,
                       height=dp(36), halign="left")
        legend.bind(width=lambda w, v: setattr(w, "text_size", (v, None)))
        content.add_widget(legend)
        scroll = ScrollView(bar_width=dp(3))
        scroll.add_widget(content)
        popup = Popup(title="Грибной радар · 7 дней", content=scroll,
                      size_hint=(0.98, 0.78), separator_color=ACCENT,
                      title_size=sp(15), background_color=CARD,
                      title_color=INK)
        popup.open()
