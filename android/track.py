...
class Walk:
    ...
    # Прогноз на день похода: {ключ вида: индекс}. Снимается один раз, при
    # старте, и больше не меняется.
    #
    # Зачем хранить, если индекс можно пересчитать потом. Во-первых, для
    # пересчёта нужна архивная погода и интернет, а карточку похода люди
    # открывают где угодно. Во-вторых и главное: пересчёт даст сегодняшнюю
    # версию модели с сегодняшними коэффициентами, а вопрос стоит другой —
    # что приложение обещало ТОГДА и чем это кончилось. Без записи в момент
    ...
    def from_dict(cls, raw: dict) -> "Walk":
        ...
        # Походы, записанные до появления снимка прогноза, читаются с пустым
        # index — и все расчёты по нему обязаны это переживать молча.
        try:
            w.index = {str(k): float(v)
                       for k, v in (raw.get("index") or {}).items()}
