...
class PlacePicker(Popup):
    ...
    def __init__(self, lat, lon, on_done, **kw):
        ...
        self.heat_species = Spinner(
            ...
            background_color=hexc(palette.SOFT), color=INK)
        self.heat_species.shorten = True
        self.heat_species.bind(size=lambda w, size: setattr(w, "text_size", (max(dp(30), size[0]-dp(18)), size[1])))
        ...
        for control in (self.heat_species, self.b_heat):
            control.background_color = (0, 0, 0, 0)
            uikit.fill_rounded(control, hexc(palette.SOFT))
        legend = Label(text="Индекс:  низкий  ·  средний  ·  высокий     авто-данные",
                       color=MUTED, font_size=sp(10), size_hint_y=None, height=dp(22))
        root.add_widget(legend)
        self.top_zones = Label(text="", color=INK, font_size=sp(11),
                               size_hint_y=None, height=dp(0), halign="left", valign="top")
        self.top_zones.bind(width=lambda w, v: setattr(w, "text_size", (v, None)))
        root.add_widget(self.top_zones)
    ...
    def _heat_progress(self, done, total):
        self.heat_status.text = f"Тепловая карта: {done}/{total} клеток ({round(100 * done / total) if total else 0}%)…"
    ...
    def _heat_done(self, grid):
        ...
        self.map.heat = grid
        self.map.redraw()
        zones = heatspots.top_areas(grid, self.lat, self.lon)
        self.top_zones.text = ("Перспективные участки (по прямой):\n" +
            "\n".join(f"{n}. {z.index:.0f}/100 · {z.distance_km:.1f} км · "
                      f"{z.biotope} · {z.relief}"
                      for n, z in enumerate(zones, 1))) if zones else ""
        self.top_zones.height = max(dp(20 + 18 * len(zones)), self.top_zones.texture_size[1] + dp(12)) if zones else dp(0)
