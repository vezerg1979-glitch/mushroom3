...
class WalkScreen(Popup):
    ...
    def on_dismiss(self):
        """Отвязка от Window.size — иначе экран продолжает жить в подписчиках.

        Без этого при каждом новом походе (открыл — закрыл — открыл снова)
        в списке слушателей Window накапливается по одному мёртвому
        обработчику на каждый закрытый экран. Каждый следующий поворот
        телефона исполняет их все — включая те, что держат ссылку на давно
        закрытый попап и трогают его виджеты вслепую. Возвращать False,
        а не оставлять пустым: True здесь отменяет закрытие (см. исходник
        ModalView.dismiss), и по ошибке можно было бы получить попап,
        который не закрывается со второго раза.
        """
    ...
    def show_service_log(self):
        ...
        b_near = Button(size_hint_y=None, height=dp(48), font_size=sp(13),
                        background_normal="",
                        background_color=hexc(palette.SOFT), color=INK)
