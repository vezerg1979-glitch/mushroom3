...
def свои_настройки(tmp_path, monkeypatch):
    ...
    Восстановление темы — через theme.apply(), а не через прямой
    palette.use(). Прямой вызов меняет текущую палитру, но не оповещает
    зарегистрированные модули (donate.py и другие «экранные» модули,
    которые держат свои цвета в кэше и обновляют его только по сигналу
    theme.register). После такого восстановления donate.INK_HEX мог
    остаться от «ночи», хотя palette.current() уже снова «день» — ровно
    так тесты в test_palette.py и test_donate.py однажды и поймали чужой
    осадок от этого файла.
    """
    import places
    ...
    yield
    # были всегда «день» либо «ночь» (palette.current() не отдаёт «авто»),
    # так что то же самое имя годится и как аргумент set_mode.
    theme.set_mode(было, LAT, LON, T)
