...
class _Method:
    ...
    def __call__(self, *a, **kw):
        ...
        if short == "runOnUiThread" and a and callable(a[0]):
            # Настоящий Android откладывает вызов на UI-поток, здесь —
            # выполняет тут же. Разница не в тайминге, который на компьютере
            # всё равно не проверить, а в том, доходит ли код внутри
            # колбэка до конца без опечаток в именах и числе аргументов —
            # ровно то, для чего эта подделка и существует.
            a[0]()
