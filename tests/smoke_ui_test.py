...
def app():
    """Один и тот же экран на весь файл. Тема закреплена днём намеренно:
    без этого MushroomApp.build() решал бы день это или ночь по режиму
    «авто» и настоящему времени суток на машине, где идёт прогон, — и весь
    набор становился бы то зелёным, то падал в зависимости от часа, когда
    его запустили. Тесты на саму автоматику темы и на ночную палитру
    живут отдельно в test_theme.py и специально фиксируют время суток,
    какое им нужно, а не полагаются на часы вслепую.
    """
    import theme
...
def test_every_reference_picture_actually_draws(key, monkeypatch):
    ...
    monkeypatch на photo_path обязателен: SpeciesPicture умеет показывать
    и настоящую фотографию вида вместо рисунка (см. android/assets/
    species/), если файл для этого вида туда положен. Тест проверяет
    именно арифметику векторной отрисовки, а не то, есть ли в данный
    момент в репозитории чья-то фотография белого гриба — эти две вещи
    не должны зависеть друг от друга. Без monkeypatch тест то проходит,
    то падает в зависимости от того, что физически лежит в assets на
    момент прогона, — ровно это и произошло, когда фотографии реальных
    грибов появились в assets/species уже после того, как тест был
    написан.
    """
    import atlas

    monkeypatch.setattr(atlas, "photo_path", lambda k: None)
...
def test_premium_button_hides_after_unlock(app, tmp_path, monkeypatch):
    """Кнопка «Без рекламы» не должна предлагать купить то, что уже куплено."""
    import premium

    assert app.b_premium is not None
    premium.unlock(source="manual")
    app._premium_unlocked()
    assert app.b_premium is None


def test_premium_screen_shows_thanks_when_already_bought():
    import premium
    import premium_screen

    premium.unlock(source="manual")
    try:
        scr = premium_screen.show()
        текст = " ".join(getattr(x, "text", "") for x in _walk(scr.content))
        assert "уже выключена" in текст
        scr.dismiss()
    finally:
        import prefs
        prefs.save(premium=False)


def test_premium_screen_shows_the_pay_and_code_flow():
    import premium
    import premium_screen

    prev = premium.is_premium()
    if prev:
        pytest.skip("уже куплено в этой сессии")
    scr = premium_screen.show()
    try:
        from kivy.uix.textinput import TextInput

        assert scr.device_label.text                  # код устройства виден
        entries = [w for w in _walk(scr.content) if isinstance(w, TextInput)]
        assert entries, "нет поля ввода кода разблокировки"
    finally:
        scr.dismiss()


def test_rustore_button_absent_while_billing_is_not_wired(app):
    """billing.is_available() сегодня всегда False — кнопки RuStore быть не
    должно, а не быть и не нажиматься."""
    from kivy.uix.button import Button
    ...
    scr = premium_screen.show()
    try:
        кнопки = [b.text for b in _walk(scr.content) if isinstance(b, Button)]
        assert not any("RuStore" in t for t in кнопки)
    finally:
        scr.dismiss()


def test_wrong_code_shows_a_message_not_a_crash():
    import premium
    import premium_screen
    ...
    scr = premium_screen.show()
    try:
        scr.entry.text = "0000-0000"
        scr._apply_code()
        assert "не подошёл" in scr.status.text
        assert premium.is_premium() is False
    finally:
        scr.dismiss()


def test_correct_code_unlocks_and_closes(app):
    import licensecode
    import premium

    if premium.is_premium():
        pytest.skip("уже куплено в этой сессии")
    scr = app.show_premium() or _popup_titled("Без рекламы за 300 руб.")
    try:
        код = licensecode.unlock_code_for(licensecode.device_code())
        scr.entry.text = код
        scr._apply_code()
        assert premium.is_premium() is True
        # dismiss() у Popup закрывается через анимацию — сразу после вызова
        # окно формально ещё открыто, закрытие завершится через несколько
        # тиков часов.
        from kivy.clock import Clock
        for _ in range(10):
            Clock.tick()
        assert scr._is_open is False
    finally:
        import prefs
        prefs.save(premium=False)
        if scr._is_open:
